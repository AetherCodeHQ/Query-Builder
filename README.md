...
> Query-Builder - Open source tool by AetherCodeHQ

`builder` `cli` `developer-tools` `golang` `query` `sql`

## What is it?

**Query Builder** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Query-Builder --path ./target

# With options
./Query-Builder --path ./target --format json --output report.json

# Verbose mode
./Query-Builder --path ./target --verbose
```
...
### Basic scan
```bash
./Query-Builder --path ./my-project
```

### JSON report
```bash
./Query-Builder --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Query Builder
  run: ./Query-Builder --path . --format json --output report.json
```
